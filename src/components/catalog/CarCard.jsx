...
export default function CarCard({ car }) {
  ...
  const next = () => setImgIdx(i => Math.min((car.imageCount || 1) - 1, i + 1))

  const totalCost = car.commission + car.delivery + car.loading + car.unloading + car.storage - car.vatRefund + car.priceUSD
    ...
    <div className="car-card">
      <div className="car-card-top">
        ...
        <div className="car-img-wrap">
          ...
          {(car.imageCount || 1) > 1 && (
            <>
              <button className="car-img-btn car-img-btn-prev" onClick={prev} disabled={imgIdx === 0}>
                <PrevIcon />
              </button>
              <button className="car-img-btn car-img-btn-next" onClick={next} disabled={imgIdx === (car.imageCount - 1)}>
                <NextIcon />
              </button>
            </>
          )}
          <span className="car-img-counter">{imgIdx + 1}/{car.imageCount || 1}</span>
          {car.encarUrl && (
            <span className="car-img-encar-badge">encar</span>
          )}
        </div>
        ...
        <div className="car-info">
          ...
          <div className="car-detail">
            ...
            <span className="car-color-dots">
              {car.bodyColorDots.map((c, i) => (
                <span key={i} className="car-color-dot" style={{ background: c }} />
              ))}
            </span>
          </div>
          <div className="car-detail">
        ...
        <div className="car-price-col">
          <div className="car-price-krw">{car.priceKRW.toLocaleString()} ₩</div>
          <div className="car-price-usd">${car.priceUSD.toLocaleString()}</div>
          <div className="car-price-breakdown">
            <div className="car-price-row">
              <span>Комиссия:</span><span>${car.commission.toLocaleString()}</span>
            </div>
            <div className="car-price-row">
              <span>Доставка:</span><span>${car.delivery.toLocaleString()}</span>
            </div>
            <div className="car-price-row">
              <span>Погрузка:</span><span>${car.loading.toLocaleString()}</span>
            </div>
            <div className="car-price-row">
              <span>Выгрузка:</span><span>${car.unloading.toLocaleString()}</span>
            </div>
            <div className="car-price-row">
              <span>Стоянка:</span><span>${car.storage.toLocaleString()}</span>
            </div>
            <div className="car-price-row car-price-vat">
              <span>Возврат НДС:</span><span>-${car.vatRefund.toLocaleString()}</span>
            </div>
          </div>
          <div className="car-price-total">
            <span>До Бишкека:</span>
            <span>${car.total.toLocaleString()}</span>
          </div>
        </div>
      ...
      <div className="car-card-actions">
        ...
        {car.canNegotiate && (
          <button className="btn-car-negotiate">Возможен торг</button>
        )}
      </div>
    </div>
  )
